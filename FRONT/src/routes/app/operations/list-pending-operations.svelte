<script lang="ts">
	import { faCancel, faCartArrowDown, faCartPlus, faCircle, faHandHoldingDollar, faInfoCircle, faSackDollar } from "@fortawesome/free-solid-svg-icons";

  import { isSignatureRequired, type OperationBasicData, type OperationStatusType, type OperationTypeType } from "$lib/domain/operations";
  import { faCheckCircle, faMoneyBillAlt, type IconDefinition, type IconLookup } from "@fortawesome/free-regular-svg-icons";
  import { Button, ButtonGroup, Checkbox, Helper, Img, Label, Modal, Table, TableBody, TableBodyCell, TableBodyRow, TableHead, TableHeadCell, Textarea, Tooltip } from 'flowbite-svelte';
  ...
  const execWithSignature = async () => {
    ...
    if (r.status !== 200) {
      ...
      showError('No se ha podido obtener la información para firmar la operación. ERROR: ' + error.message || error || r.statusText);
      return;
    }
    ...
    <TableBodyRow class="divide-y">
      ...
      <TableBodyCell>
        ...
        {#if op.status === 'pending'}
          <Button color="primary" size="sm" outline={true} pill={true} on:click={() => openExecDialog(op)}>
            <Fa icon={faCheckCircle} size="lg" />
          </Button>
          {#if op.type !== 'notification'}        
            <Tooltip>Ejecuta la operación</Tooltip>
            <Button color="dark" size="sm" outline={true} pill={true} on:click={() => openRejectionDialog(op)} >
...
<Modal title="Rechazar operación" bind:open={rejectModal} >
  <div class="flex flex-row gap-5 w-full">
    ...
    <div  class="flex flex-col gap-2 grow">
      ...
      <p class="text-base leading-relax flex flex-row gap-2">
        <span class="text-md">Valor de venta: <strong>{selectedOperation?.salePrice}€</strong></span>
      </p>
      {/if}
      <div>
        <Label for="rejectedReason" class="text-gray-500" >Motivo del rechazo</Label>
        <Textarea bind:value={rejectionReason} class="w-full" name="rejectedReason" id="rejectedReason" placeholder="Indica el motivo de rechazo" required="true" rows="3" />
      </div>
    </div>
  </div>
  <div>
    <Checkbox bind:checked={acceptRejection} color="red" aria-describedby="helper-reject-check">Entiendo que si continúo, la acción no se puede deshacer</Checkbox>
    <Helper id="helper-reject-check" class="ps-6">Al rechazar la operación ya no será posible ejecutarla en un futuro</Helper>
  </div>

  <svelte:fragment slot="footer">
    <Button color="red" on:click={rejectOperation}>Rechazar</Button>
    <Button color="alternative" on:click={() => {rejectModal = false}}>Cancelar</Button>
  </svelte:fragment>
...
<Modal title="Aceptar operación" bind:open={executionModal} >
  ...
  {#if selectedOperation?.type !== 'notification'}
    <div>
      <Checkbox bind:checked={acceptExecution} color="primary" aria-describedby="helper-accept-check">Entiendo que si continúo, la acción no se puede deshacer</Checkbox>
      <Helper id="helper-accept-check" class="ps-6">{aceptTextByType(selectedOperation?.type)}</Helper>
    </div>
  {/if}

  <svelte:fragment slot="footer">
    <Button color="primary" on:click={execOperation}>Aceptar</Button>
    <Button color="alternative" on:click={() => {executionModal = false}}>Cancelar</Button>
  </svelte:fragment>
