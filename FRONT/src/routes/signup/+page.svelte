...
<script lang="ts">
	...
	import { faAddressBook, faEnvelope, faPlayCircle, faUser } from '@fortawesome/free-regular-svg-icons';
			...
			<div class="p-6 space-y-4 md:space-y-6 sm:p-8">
				...
				<form class="space-y-4 md:space-y-6" on:submit|preventDefault={onSubmit} >
					<div>
						...
						<Input type="text" name="fullName" id="fullName" placeholder="John Doe" required="true" size="lg" >
							<Fa icon={faUser} slot="left" />    
						</Input>
					</div>
					<div>
						<Label for="email" class="text-gray-500" >Email</Label>
						<Input type="email" name="email" id="email" placeholder="name@example.com" required="true" size="lg" >
					...
					<p class="text-sm font-light text-gray-500 dark:text-gray-400">
						<Checkbox id="terms" name="terms" required="true" >
							<a id="terms_and_conditions" class="font-medium text-primary-600 hover:underline ">Condiciones legales</a>
						</Checkbox>
						<Popover class="w-64 text-sm font-light " title="Términos y condiciones" triggeredBy="#terms_and_conditions">{terms}</Popover>
					</p>
					<Button type="submit" class="w-full bg-secondary-600 hover:bg-secondary-500" disabled='{!$walletAccount}'> 
